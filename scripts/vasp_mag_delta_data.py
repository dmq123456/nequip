...
from .vasp_mlff_bec import judge_same_atoms
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description=textwrap.dedent(
            """
                ...
                Example:
                    nequip-data-delta_mag -p ./rand_1/result/ -d ./fix_spin/result/ -z 53 -c 4 -o ./data.extxyz
            """
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-d",
        "--deltapath",
        help="delta learning folder path corresponding to `path`, example: for path/OUTCAR.1, the fix-spin energy label related to deltapath/OUTCAR.1, default: ./",
        type=str,
        default="./",
    )
    parser.add_argument(
    ...
    if cpu == 1:
        for i in trange(len(outcar_paths)):
            ...
            atoms, lconvergence = read_vasp_atom(
                ...
                lmagnetization_data=True,
                lread_born=False,
                lread_piezo=False,
                lforce_convergence=lforce_convergence,
                lread_OSZICAR=True,
            )
            path_fix_spin = os.path.join(delta_path, os.path.basename(path))
            if os.path.exists(path_fix_spin):
                atoms_fix_spin, lconvergence_fix_spin = read_vasp_atom(
                    path_fix_spin,
                    lforce_convergence=lforce_convergence,
                )
                if lconvergence_fix_spin and lconvergence:
                    assert judge_same_atoms(
                        atoms, atoms_fix_spin
                    ), f"atoms in {path_fix_spin} and {path} are not the same"
                    atoms.info["energy_fix_spin"] = atoms_fix_spin.get_total_energy()
                    atoms.info["energy_delta_spin"] = atoms._calc.results["energy"] - atoms.info["energy_fix_spin"]

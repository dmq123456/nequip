...
TENSOR_9_LABELS = ("xx", "xy", "xz", "yx", "yy", "yz", "zx", "zy", "zz")
...
def main(args=None, running_as_script: bool = True):
    ...
    parser.add_argument(
        "--test-validation",
        help="If used and --test-indexes not used, all data frames *not* used as training in the training session `train_dir` will be used. (validation data + test data)",
        action="store_true",
    )
    parser.add_argument(
        "--batch-size",
    ...
    parser.add_argument(
        "--component",
        help="Additionally report per-component metrics and parity/error PNGs for vector/tensor outputs.",
        action="store_true",
    )
    parser.add_argument(
        "--log",
    ...
    logger = logging.getLogger("nequip-evaluate-component")
    ...
    if (
        ...
        if dataset_is_validation:
            ...
            if args.test_validation:
                test_idcs = list(all_idcs - train_idcs)
                logger.info(
                    f"Using origial training dataset ({len(dataset)} frames) minus training ({len(train_idcs)} frames)), yielding a test set size of {len(test_idcs)} frames.",
                )
            else:
    ...
    if do_metrics:
        ...
        component_dft_value = None
        if args.component:
            component_metrics = Metrics(
                components=build_component_metrics_components(metrics_components)
            )
            component_metrics.to(device=device)
            component_predict_value = {}
            component_dft_value = {}
        ...
        for key in out_key:
            ...
            if args.component:
                component_predict_value[key] = []
                component_dft_value[key] = []
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                if do_metrics:
                    ...
                    # Store output value
                    for key in out_key:
                        predict_value[key] = torch.cat(
                            [
                                predict_value[key].view(-1),
                                out[key].view(-1).detach().cpu(),
                            ],
                            dim=0,
                        )
                        dft_value[key] = torch.cat(
                            [
                                dft_value[key].view(-1),
                                batch[key].view(-1).detach().cpu(),
                            ],
                            dim=0,
                        )
                        if args.component:
...
def build_component_metrics_components(components):
...
def flatten_component_metrics(metrics: Metrics, type_names=None):
    ...
    for (key, param_hash), value in current.items():
        reduction, params = metrics.params[key][param_hash]
        stat = metrics.running_stats[key][param_hash]
        shape = tuple(stat.output_dim)
        if len(shape) == 0 or math.prod(shape) <= 1:
            continue

        short_name = ABBREV.get(key, key)
        func = metrics.funcs[key][param_hash]
        ...
        if params["PerSpecies"]:
            ...
            if species_names is None:
                species_names = [str(index) for index in range(len(value))]
            flat_value = value.reshape(value.shape[0], -1)
            for species_name, row in zip(species_names, flat_value):
                for label, component_value in zip(labels, row):
                    flat_dict[f"{species_name}_{item_name}_{label}"] = component_value.item()
        else:
            for label, component_value in zip(labels, value.reshape(-1)):
                flat_dict[f"{item_name}_{label}"] = component_value.item()
    return flat_dict
...
def component_labels(shape):
    if shape == (3,):
        return list(AXIS_LABELS)
    if shape == (6,):
        return list(VOIGT_LABELS)
    if shape == (9,):
        return list(TENSOR_9_LABELS)
    if len(shape) > 0 and all(dim == 3 for dim in shape):
        return [
            "".join(indices)
            for indices in itertools.product(AXIS_LABELS, repeat=len(shape))
        ]
    return [f"c{idx}" for idx in range(math.prod(shape))]


# parity plots on predicted vs. true
...
def plot_error(pred, true, key, dict="png_error"):
    import matplotlib.pyplot as plt
    import os
    import scienceplots

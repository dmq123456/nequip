...
_DEFAULT_NODE_FIELDS: Set[str] = {
    ...
    AtomicDataDict.SPIN_LENGTH_KEY,
    AtomicDataDict.SPIN_NONLEAF_KEY,
    AtomicDataDict.SPIN_FORCE_KEY,
    AtomicDataDict.SPIN_FORCE_VERT_KEY,
    AtomicDataDict.ATOM_VIRIAL_KEY,
    AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY,
    AtomicDataDict.ATOMIC_DIPOLE_KEY,
    AtomicDataDict.NODE_SPINANI_MATRIX_KEY,
}
_DEFAULT_EDGE_FIELDS: Set[str] = {
    ...
    AtomicDataDict.EDGE_NONSCALAR_FEATURES_KEY,
    AtomicDataDict.EDGE_ENERGY_KEY,
    AtomicDataDict.EDGE_HB_COEFF_KEY,
    AtomicDataDict.EDGE_BIQUAD_COEFF_KEY,
    AtomicDataDict.EDGE_SPIN_MATRIX_KEY,
    AtomicDataDict.EDGE_HB_MATRIX_KEY,
    AtomicDataDict.EDGE_SISJ_KEY,
}
_DEFAULT_GRAPH_FIELDS: Set[str] = {
    AtomicDataDict.TOTAL_ENERGY_KEY,
    AtomicDataDict.TOTAL_ENERGY_FIX_SPIN_KEY,
    AtomicDataDict.TOTAL_ENERGY_DELTA_SPIN_KEY,
    AtomicDataDict.STRESS_KEY,
    AtomicDataDict.VIRIAL_KEY,
    AtomicDataDict.PBC_KEY,
    AtomicDataDict.CELL_KEY,
    AtomicDataDict.TOTAL_BORN_EFFECTIVE_CHARGE_KEY,
    AtomicDataDict.PIEZOELECTRIC_KEY,
...
class AtomicData(Data):
    ...
    def __init__(
        ...
        if "spin" in self and self.spin is not None:
            spin_norm = torch.sum(self.spin**2, dim=-1).unsqueeze(-1)
            self.spin = torch.nn.functional.normalize(self.spin) * (spin_norm > 1e-2)

        if (
            AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY in self
            and self[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY] is not None
        ):
            if "batch" in self and self.batch is not None:
                self[AtomicDataDict.TOTAL_BORN_EFFECTIVE_CHARGE_KEY] = torch.zeros(
                    [self.batch.max() + 1, 9]
                ).type_as(self.pos)
            else:
                self[AtomicDataDict.TOTAL_BORN_EFFECTIVE_CHARGE_KEY] = torch.zeros(
        ...
        if _validate:
            ...
            if "spin" in self and self.spin is not None:
                assert self.spin.dim() == 2 and self.spin.shape[1] == 3
                assert self.spin.dtype == self.pos.dtype
            ...
            if "batch" in self and self.batch is not None:
                ...
                if AtomicDataDict.PIEZOELECTRIC_KEY in self and self[AtomicDataDict.PIEZOELECTRIC_KEY] is not None:
                    piezo = self[AtomicDataDict.PIEZOELECTRIC_KEY].view(-1, 3, 3, 3)
                    assert piezo.shape[0] == self.batch.max() + 1
                if AtomicDataDict.TOTAL_DIPOLE_KEY in self and self[AtomicDataDict.TOTAL_DIPOLE_KEY] is not None:
                    total_dipole = self[AtomicDataDict.TOTAL_DIPOLE_KEY].view(-1, 3)
                    assert total_dipole.shape[0] == self.batch.max() + 1

            if (
                AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY in self
                and self[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY] is not None
            ):
                assert self[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY].shape == (
                    self.num_nodes,
                    9,
                )  # All flatten
                t_bec = self[AtomicDataDict.TOTAL_BORN_EFFECTIVE_CHARGE_KEY]
                if "batch" in self and self.batch is not None:
                    assert t_bec.shape == (self.batch.max() + 1, 9)
                else:
                    assert t_bec.shape == (9,)
            
            if (
                AtomicDataDict.PIEZOELECTRIC_KEY in self
                and self[AtomicDataDict.PIEZOELECTRIC_KEY] is not None
            ):
                assert (self[AtomicDataDict.PIEZOELECTRIC_KEY].shape == (3, 3, 3)) or (
                    self[AtomicDataDict.PIEZOELECTRIC_KEY].dim() == 4 and self[AtomicDataDict.PIEZOELECTRIC_KEY].shape[1:] == (3, 3, 3)
                )
            
            if (
                AtomicDataDict.TOTAL_DIPOLE_KEY in self
                and self[AtomicDataDict.TOTAL_DIPOLE_KEY] is not None
            ):
                assert (self[AtomicDataDict.TOTAL_DIPOLE_KEY].shape == (3,)) or (
                    self[AtomicDataDict.TOTAL_DIPOLE_KEY].dim() == 2 and self[AtomicDataDict.TOTAL_DIPOLE_KEY].shape[1:] == (3,)
                )

            if (
                AtomicDataDict.ATOMIC_DIPOLE_KEY in self
                and self[AtomicDataDict.ATOMIC_DIPOLE_KEY] is not None
            ):
                assert self[AtomicDataDict.ATOMIC_DIPOLE_KEY].shape == (self.num_nodes,3)
    ...
    def from_ase(
        ...
        **kwargs,
        ...
        km = {
            ...
            "energy_fix_spin": AtomicDataDict.TOTAL_ENERGY_FIX_SPIN_KEY,
            "energy_delta_spin": AtomicDataDict.TOTAL_ENERGY_DELTA_SPIN_KEY,
            "initial_magmoms": AtomicDataDict.SPIN_KEY,
            "spin_length": AtomicDataDict.SPIN_LENGTH_KEY,
            "spin_forces": AtomicDataDict.SPIN_FORCE_KEY,
            "spin_forces_vert": AtomicDataDict.SPIN_FORCE_VERT_KEY,
            "hb_coeff": AtomicDataDict.EDGE_HB_COEFF_KEY,
            "biquad_coeff": AtomicDataDict.EDGE_BIQUAD_COEFF_KEY,
            "hb_matrix": AtomicDataDict.EDGE_HB_MATRIX_KEY,
            "node_ani_matrix": AtomicDataDict.NODE_SPINANI_MATRIX_KEY,
            "dipole": AtomicDataDict.TOTAL_DIPOLE_KEY,
            "atomic_dipole": AtomicDataDict.ATOMIC_DIPOLE_KEY,
        }
        ...
        # handle Born Effective Charge
        for key in AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY:
            if key in add_fields:
                if add_fields[key].shape[-2:] == (3, 3):
                    add_fields[key] = add_fields[key].reshape(-1, 9)
                elif add_fields[key].shape[-1] == (9):
                    # it's already 9, do nothing else
                    pass
                else:
                    raise RuntimeError(f"bad shape for {key}")
    
        
        # handle piezoelectric
        for key in AtomicDataDict.PIEZOELECTRIC_KEY:
            if key in add_fields:
                if add_fields[key].shape[-3:] == (3, 3, 3):
                    add_fields[key] = add_fields[key].reshape(-1, 3, 3, 3)
                elif add_fields[key].shape[-1] == (27):
                    add_fields[key] = add_fields[key].reshape(-1, 3, 3, 3)
                else:
                    raise RuntimeError(f"bad shape for {key}")
    ...
    def to_ase(
        ...
        spin = getattr(self, AtomicDataDict.SPIN_KEY, None)
        spin_length = getattr(self, AtomicDataDict.SPIN_LENGTH_KEY, None)
        spin_force = getattr(self, AtomicDataDict.SPIN_FORCE_KEY, None)
        spin_force_vert = getattr(self, AtomicDataDict.SPIN_FORCE_VERT_KEY, None)
        atomic_dipole = getattr(self, AtomicDataDict.ATOMIC_DIPOLE_KEY, None)
        dipole = getattr(self, AtomicDataDict.TOTAL_DIPOLE_KEY, None)
        do_calc = any(
            k in self
            for k in [
                AtomicDataDict.TOTAL_ENERGY_KEY,
        ...
        for batch_idx in range(n_batches):
            ...
            mol = ase.Atoms(
                ...
                magmoms=spin[mask] if spin is not None else None,
            )
            ...
            if do_calc:
                ...
                if spin_force is not None:
                    fields["spin_forces"] = spin_force[mask].cpu().numpy()
                if spin_force_vert is not None:
                    fields["spin_forces_vert"] = spin_force_vert[mask].cpu().numpy()
                if atomic_dipole is not None:
                    fields["atomic_dipole"] = atomic_dipole[mask].cpu().numpy()
                if dipole is not None:
                    fields["dipole"] = dipole[batch_idx].cpu().numpy()
                if AtomicDataDict.STRESS_KEY in self:
                    fields["stress"] = full_3x3_to_voigt_6_stress(
                        self["stress"].view(-1, 3, 3)[batch_idx].cpu().numpy()
                    )
                if AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY in self:
                    fields["born_effective_charges"] = (
                        self["born_effective_charges"][mask]
                        .view(-1, 3, 3)
                        .cpu()
                        .numpy()
                    )
                if AtomicDataDict.PIEZOELECTRIC_KEY in self:
                    fields["piezoelectric"] = (
                        self["piezoelectric"][batch_idx]
                        .view(-1, 3, 3, 3)
                        .cpu()
                        .numpy()
                    )

...
class TypeMapper:
    ...
    def __init__(
        ...
        chemical_symbols: Optional[Union[List[str], str]] = None,
    ):
        if chemical_symbols is not None:
            if chemical_symbols[0] == "universal":
                if len(chemical_symbols) == 1:
                    max_atomic_num = 100
                else:
                    assert len(chemical_symbols) == 2
                    max_atomic_num = int(chemical_symbols[1])
                atomic_nums = list(range(1, max_atomic_num + 1))
                chemical_symbols = [
                    ase.data.chemical_symbols[i] for i in range(max_atomic_num)
                ]
            else:

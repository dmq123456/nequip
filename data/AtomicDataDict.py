...
@torch.jit.script
def with_edge_sisj(data: Type) -> Type:
    """Compute the edge si dot sj.

    Returns:
        Tensor [n_edges, 1] edge displacement vectors
    """
    if _keys.EDGE_SISJ_KEY in data:
        return data
    else:
        if _keys.SPIN_NONLEAF_KEY not in data:
            data[_keys.SPIN_NONLEAF_KEY] = data[_keys.SPIN_KEY] # torch.nn.functional.normalize(data[_keys.SPIN_KEY], dim=-1)
        spin = data[_keys.SPIN_NONLEAF_KEY]
        col = data[_keys.EDGE_INDEX_KEY][1]
        row = data[_keys.EDGE_INDEX_KEY][0]
        data[_keys.EDGE_SISJ_KEY] = torch.sum(spin[row] * spin[col], dim=-1).view(-1, 1)
        return data

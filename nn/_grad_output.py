...
class GradientOutput(GraphModuleMixin, torch.nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        grads = torch.autograd.grad(
            ...
            retain_graph=self.retain_graph or self.training,
        )
        # return
        ...
        for out, grad in zip(self.out_field, grads):
            ...
            if self._negate:
                grad = torch.neg(grad)
            data[out] = grad
            if out == AtomicDataDict.SPIN_FORCE_KEY:
                data[out] = data[out] * torch.sum(
                    data[AtomicDataDict.SPIN_KEY] ** 2, dim=-1
                ).unsqueeze(-1)
...
@compile_mode("script")
class ForceSpinForceOutput(GraphModuleMixin, torch.nn.Module):
    r"""Wrap a model and include as an output its gradient.
    ...
    def __init__(
        ...
        super().__init__()
        self.of = AtomicDataDict.TOTAL_ENERGY_KEY
        self.wrt = [AtomicDataDict.SPIN_KEY, AtomicDataDict.POSITIONS_KEY]
        self.out_field = [AtomicDataDict.SPIN_FORCE_KEY, AtomicDataDict.FORCE_KEY]
        ...
        self._init_irreps(
            irreps_in=func.irreps_in,
            my_irreps_in={self.of: Irreps("0e")},
            irreps_out=func.irreps_out,
            include_spin=True,
        )
...
@compile_mode("script")
class ParaStressBECOutput(GraphModuleMixin, torch.nn.Module):
    r"""Compute stress, atomic virial, forces, Born Effective charge using autograd of an energy and dipole model.
    Edge-vector based atomic virial.
    Design for Lammps parallism.
    ...
    def __init__(
        ...
        self.irreps_out[AtomicDataDict.VIRIAL_KEY] = "3x1o"
        self.irreps_out[AtomicDataDict.ATOM_VIRIAL_KEY] = "3x1o"
        self.irreps_out[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY] = "3x1o"
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        for i in range(3):
            ...
            if bec_grads is None:
                # condition needed to unwrap optional for torchscript
                assert False, "failed to compute bec autograd"
            bec_grads_list.append(bec_grads)

        bec = torch.stack(bec_grads_list, dim=-1)

        # grads_bec = torch.autograd.grad(
        #     [data[AtomicDataDict.ATOMIC_DIPOLE_KEY]],
        #     [pos],
        #     create_graph=self.training,  # needed to allow gradients of this output during training
        # )
        # bec = grads_bec[0]
        data[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY] = bec.transpose(1,2).view(-1, 9)
...
@compile_mode("unsupported")
class PartialSpinForceOutput(GraphModuleMixin, torch.nn.Module):
    r"""Generate partial and total spin forces from an energy model.
    dE/dS
    ...
    def __init__(
        ...
        self.irreps_out[AtomicDataDict.SPIN_PARTIAL_FORCE_KEY] = Irreps("1eo")
        self.irreps_out[AtomicDataDict.SPIN_FORCE_KEY] = Irreps("1eo")

    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        data = data.copy()
        out_data = {}

        def wrapper(spin: torch.Tensor) -> torch.Tensor:
            """Wrapper from spin to atomic energy"""
            nonlocal data, out_data
        ...
        out_data[AtomicDataDict.SPIN_PARTIAL_FORCE_KEY] = partial_forces
        out_data[AtomicDataDict.SPIN_FORCE_KEY] = -partial_forces.sum(
            dim=0
        ) * torch.sum(data[AtomicDataDict.SPIN_KEY] ** 2, dim=-1).unsqueeze(-1)

        return out_data


@compile_mode("script")
class StressForceSpinOutput(GraphModuleMixin, torch.nn.Module):
    r"""Compute stress, forces and spin forces using autograd of an energy model.
    ...
    def __init__(
        ...
        self.irreps_out[AtomicDataDict.FORCE_KEY] = "1o"
        self.irreps_out[AtomicDataDict.SPIN_FORCE_KEY] = "1eo"
        self.irreps_out[AtomicDataDict.STRESS_KEY] = "3x1o"
        self.irreps_out[AtomicDataDict.VIRIAL_KEY] = "3x1o"
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        did_pos_req_grad: bool = pos.requires_grad
        pos.requires_grad_(True)
        did_spin_req_grad: bool = spin.requires_grad
        ...
        grads = torch.autograd.grad(
            [data[AtomicDataDict.TOTAL_ENERGY_KEY].sum()],
            [pos, data["_displacement"], data[AtomicDataDict.SPIN_KEY]],
            create_graph=self.training,  # needed to allow gradients of this output during training
        )
        ...
        # Put negative sign on spin forces
        spin_forces = grads[2]
        if spin_forces is None:
            # condition needed to unwrap optional for torchscript
            assert False, "failed to compute spin forces autograd"
        spin_forces = torch.neg(spin_forces)
        data[AtomicDataDict.SPIN_FORCE_KEY] = spin_forces * torch.sum(
            data[AtomicDataDict.SPIN_KEY] ** 2, dim=-1
        ).unsqueeze(-1)
...
@compile_mode("script")
class ParaStressForceSpinOutput(GraphModuleMixin, torch.nn.Module):
    r"""Compute stress, forces and spin forces using autograd of an energy model.
    ...
    def __init__(
        ...
        self.irreps_out[AtomicDataDict.FORCE_KEY] = "1o"
        self.irreps_out[AtomicDataDict.SPIN_FORCE_KEY] = "1eo"
        self.irreps_out[AtomicDataDict.SPIN_FORCE_VERT_KEY] = "1eo"
        self.irreps_out[AtomicDataDict.STRESS_KEY] = "3x1o"
        self.irreps_out[AtomicDataDict.VIRIAL_KEY] = "3x1o"
        self.irreps_out[AtomicDataDict.ATOM_VIRIAL_KEY] = "3x1o"
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        # Put negative sign on spin forces here
        spin_forces = grads[2]
        if spin_forces is None:
            # condition needed to unwrap optional for torchscript
            assert False, "failed to compute spin forces autograd"
        spin_forces = torch.neg(spin_forces)
        spin_forces = spin_forces * torch.sum(
            data[AtomicDataDict.SPIN_KEY] ** 2, dim=-1
        ).unsqueeze(-1)
        data[AtomicDataDict.SPIN_FORCE_KEY] = spin_forces
        spin_forces_vec = (
            spin_forces - torch.sum(spin_forces * spin, dim=-1).view(-1, 1) * spin
        )  # Project spin_force into direction that vectical to spin
        data[AtomicDataDict.SPIN_FORCE_VERT_KEY] = spin_forces_vec

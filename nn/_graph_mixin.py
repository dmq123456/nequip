...
class GraphModuleMixin:
    ...
    def _init_irreps(
        ...
        if include_spin:
            # If include spin, spin are also always present.
            if AtomicDataDict.SPIN_KEY in irreps_in:
                if irreps_in[AtomicDataDict.SPIN_KEY] != o3.Irreps("1x1eo"):
                    raise ValueError(
                        f"Spin must have irreps 1eo, got instead `{irreps_in[AtomicDataDict.SPIN_KEY]}`"
                    )
            irreps_in[AtomicDataDict.SPIN_KEY] = o3.Irreps("1eo")
            if AtomicDataDict.SPIN_NONLEAF_KEY in irreps_in:
                if irreps_in[AtomicDataDict.SPIN_NONLEAF_KEY] != o3.Irreps("1x1eo"):
                    raise ValueError(
                        f"Spin must have irreps 1eo, got instead `{irreps_in[AtomicDataDict.SPIN_NONLEAF_KEY]}`"
                    )

...
def PerSpeciesRescale(
    ...
    # == In delta learning, the TOTAL_ENERGY_FIX_SPIN_KEY should be consistent with TOTAL_ENERGY_KEY in operation==
    ldelta_learning = config.get("ldelta_learning", False)
    if ldelta_learning:
        params["scales_trainable"] = False
        params["shifts_trainable"] = False
        logging.debug("In delta learning of magnetic dataset, scales & shifts are not trainable.")
        model.insert_from_parameters(
            before="energy_sum_fix_spin",
            name=module_prefix+"fix_spin",

...
class NequIPCalculator(Calculator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spin_units_to_ub = spin_units_to_ub
    ...
    def calculate(self, atoms=None, properties=["energy"], system_changes=all_changes):
        ...
        if AtomicDataDict.SPIN_FORCE_KEY in out:
            # force has units eng / len:
            self.results["spin_forces"] = (
                self.energy_units_to_eV / self.spin_units_to_ub
            ) * out[AtomicDataDict.SPIN_FORCE_KEY].detach().cpu().numpy()
        if AtomicDataDict.SPIN_FORCE_VERT_KEY in out:
            # force has units eng / len:
            self.results["spin_forces_vert"] = (
                self.energy_units_to_eV / self.spin_units_to_ub
            ) * out[AtomicDataDict.SPIN_FORCE_VERT_KEY].detach().cpu().numpy()
        ...
        if AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY in out:
            bec = out[AtomicDataDict.BORN_EFFECTIVE_CHARGE_KEY].detach().cpu().numpy()
            bec = bec.reshape(-1, 3, 3) * (
                self.energy_units_to_eV / self.length_units_to_A  # TODO: Related units
            )
            self.results["born_effective_charges"] = bec
        if AtomicDataDict.PIEZOELECTRIC_KEY in out:
            piezo = out[AtomicDataDict.PIEZOELECTRIC_KEY].detach().cpu().numpy()
            piezo = piezo.reshape(-1, 3, 3, 3) * (
                self.energy_units_to_eV * self.length_units_to_A  # TODO: Related units
            )
            self.results["piezoelectric"] = piezo
        if AtomicDataDict.ATOMIC_DIPOLE_KEY in out:
            atomic_dipole = out[AtomicDataDict.ATOMIC_DIPOLE_KEY].detach().cpu().numpy()
            atomic_dipole = atomic_dipole.reshape(-1, 3) * (
                self.energy_units_to_eV * self.length_units_to_A  # TODO: Related units
            )
            self.results["atomic_dipole"] = atomic_dipole
        if AtomicDataDict.TOTAL_DIPOLE_KEY in out:
            dipole = out[AtomicDataDict.TOTAL_DIPOLE_KEY].detach().cpu().numpy()

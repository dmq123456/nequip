...
def assert_AtomicData_equivariant(
    ...
    **kwargs,
    ...
    r"""Test the rotation, translation, parity, time reversal and permutation equivariance of ``func``.
    ...
    Args:
        ...
        do_parity: whether test parity
        do_time_reversal: whether test time reversal
        do_only_rot_spin: whether only rot spin while other keep the same. Used in tests for not considering spin orbital coupling.
    ...
    for irps in (irreps_in, irreps_out):
        ...
        if AtomicDataDict.SPIN_KEY in irps:
            # it should always have been 1eo vectors
            # since that's actually a valid Irreps
            assert o3.Irreps(irps[AtomicDataDict.SPIN_KEY]) == o3.Irreps("1eo")
            irps[AtomicDataDict.SPIN_KEY] = "spin"
        if AtomicDataDict.SPIN_NONLEAF_KEY in irps:
            # it should always have been 1eo vectors
            # since that's actually a valid Irreps
            assert o3.Irreps(irps[AtomicDataDict.SPIN_NONLEAF_KEY]) == o3.Irreps("1eo")
            irps[AtomicDataDict.SPIN_NONLEAF_KEY] = "spin"
        if AtomicDataDict.SPIN_FORCE_KEY in irps:
            assert o3.Irreps(irps[AtomicDataDict.SPIN_FORCE_KEY]) == o3.Irreps("1eo")
            irps[AtomicDataDict.SPIN_FORCE_KEY] = "spin"
        if AtomicDataDict.SPIN_FORCE_VERT_KEY in irps:
            assert o3.Irreps(irps[AtomicDataDict.SPIN_FORCE_VERT_KEY]) == o3.Irreps("1eo")
            irps[AtomicDataDict.SPIN_FORCE_VERT_KEY] = "spin"
        if AtomicDataDict.SPIN_PARTIAL_FORCE_KEY in irps:
            assert o3.Irreps(irps[AtomicDataDict.SPIN_PARTIAL_FORCE_KEY]) == o3.Irreps(
                "1eo"
            )
            irps[AtomicDataDict.SPIN_PARTIAL_FORCE_KEY] = "spin"
    ...
    for k in born_key:
        irreps_in.pop(k, None)
    if any(k in irreps_out for k in born_key):
        from e3nn.io import CartesianTensor

        born_cart_tensor = CartesianTensor("ij=ij")
        born_rtp = born_cart_tensor.reduced_tensor_products().to(device, dtype)
        for k in born_key:
            if k in irreps_out:
                irreps_out[k] = born_cart_tensor
    ...
    for k in ani_key:
        irreps_in.pop(k, None)
    if any(k in irreps_out for k in ani_key):
        from e3nn.io import CartesianTensor

        ani_cart_tensor = CartesianTensor("ij=ji")
        ani_rtp = ani_cart_tensor.reduced_tensor_products().to(device, dtype)
        for k in ani_key:
            if k in irreps_out:
                irreps_out[k] = ani_cart_tensor
    
    piezo_key = (AtomicDataDict.PIEZOELECTRIC_KEY,)
    for k in piezo_key:
        irreps_in.pop(k, None)
    if any(k in irreps_out for k in piezo_key):
        from e3nn.io import CartesianTensor

        piezo_cart_tensor = CartesianTensor("ijk=ikj")
        piezo_rtp = piezo_cart_tensor.reduced_tensor_products().to(device, dtype)
        for k in piezo_key:
            if k in irreps_out:
                irreps_out[k] = piezo_cart_tensor
    ...
    def wrapper(*args):
        ...
        for k in born_key:
            if k in output:
                output[k] = born_cart_tensor.from_cartesian(
                    output[k].view(-1, 3, 3), rtp=born_rtp
                )

        for k in ani_key:
            if k in output:
                output[k] = ani_cart_tensor.from_cartesian(
                    output[k].view(-1, 3, 3), rtp=ani_rtp
                )

        for k in piezo_key:
            if k in output:
                output[k] = piezo_cart_tensor.from_cartesian(
                    output[k].view(-1, 3, 3, 3), rtp=piezo_rtp
                )
    ...
    errs = [
        equivariance_error(
            ...
            do_parity=do_parity,
            do_time_reversal=do_time_reversal,
            do_only_rot_spin=do_only_rot_spin,
            **kwargs,
        )
        for d in data_in
    ]
...
def add_spin(atomic_batch, spin_norm=1, spin_precent=0.8):
    # Randomly add spin
    assert spin_precent <= 1.0
    data_in = []
    for data in atomic_batch:
        natom = len(data[AtomicDataDict.POSITIONS_KEY])
        device = data[AtomicDataDict.POSITIONS_KEY].device
        spin_bool = torch.zeros((natom, 1), device=device, dtype=torch.bool)
        spin_bool[: int(natom * spin_precent)] = True
        data[AtomicDataDict.SPIN_KEY] = (
            torch.nn.functional.normalize(
                torch.rand_like(data[AtomicDataDict.POSITIONS_KEY])
            )
            * spin_norm
            * spin_bool
        )
